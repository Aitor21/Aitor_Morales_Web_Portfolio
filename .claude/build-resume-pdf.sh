...
OUT_WIN="$(cygpath -w "$ROOT/Aitor_Morales_Resume.pdf" 2>/dev/null || echo "$ROOT/Aitor_Morales_Resume.pdf")"

# 1. Temp copy whose internal links point at the PUBLIC site, so the PDF's links
#    work for a recruiter (not at localhost).
sed "s|href=\"./projects/|href=\"$PUBLIC/projects/|g" "$ROOT/resume.html" > "$ROOT/_resume_print.html"
...
# 3. Clean up
rm -f "$ROOT/_resume_print.html"

echo "Built: Aitor_Morales_Resume.pdf"
